                ...
                req.on('end', async () => {
                    try {
                        ...
                        if (webhook_notification_data.type === 'Add') {
                            ...
                            console.log(`webhook_notification_received target=${target ?? ''} fetch_target=${fetchTarget ?? ''}`);
                            ...
                            if (!latest_event_response.ok) {
                                ...
                                console.log(`webhook_notification_fetch_failed target=${target ?? fetchTarget} fetch_target=${fetchTarget} status=${latest_event_response.status}`);
                                console.log(`authorization_denied_event_not_processed target=${target ?? fetchTarget} fetch_target=${fetchTarget} status=${latest_event_response.status}`);
                                this.logger.warn({ topic, target, fetch_target: fetchTarget, status: latest_event_response.status }, 'webhook_notification_fetch_failed');
                                this.logger.warn({ topic, target: target || fetchTarget, fetch_target: fetchTarget, status: latest_event_response.status }, 'authorization_denied_event_not_processed');
                                return;
                            }
