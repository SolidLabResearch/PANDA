...
const useMax = process.env.USE_MAX === '1';
...
SELECT ${useMax ? '(MAX(?spo2Value) AS ?maxSpO2)' : '?s ?spo2Value'}
FROM NAMED WINDOW :w1 ON STREAM <http://localhost:3000/alice/spo2/> [RANGE 5000 STEP 1000]
WHERE {
  WINDOW :w1 {
...
{ ?s saref:hasValue ${useMax ? '?maxSpO2' : '?spo2Value'} . ${useMax ? '?maxSpO2' : '?spo2Value'} math:lessThan 90. } => { ?s ex:alert \"SPO2_LOW\". }.`;
