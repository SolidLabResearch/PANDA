...
// This is problematic when claims and OIDC tokens have to be passed. It might be worth looking deeper into ODRL requests to carry these claims instead of an UMA token.`))
  ...
  console.log(terms.resources.smartwatch);
  console.log(tokenParams.token_type);
  console.log(tokenParams.access_token);

  if (!tokenParams.access_token) { console.log(`No access token received...`); throw 0; }
  ...
  if (accessWithTokenResponse.status !== 200) { console.log(`Access with token failed...`); throw 0; }
