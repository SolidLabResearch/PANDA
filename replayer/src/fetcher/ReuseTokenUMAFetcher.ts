...
export class ReuseTokenUMAFetcher {
    ...
    constructor(claim: Claim) {
        ...
        this.benchmarkMode = ['1', 'true', 'yes', 'on'].includes((process.env.REPLAYER_UMA_BENCHMARK_MODE || 'false').toLowerCase());
    }
    ...
    public async fetch(url: string, init: RequestInit = {}): Promise<Response> {
        ...
        if (noTokenResponse.ok) {
            if (this.benchmarkMode) {
                throw new Error(`[Fetcher] Benchmark guard: ${url} returned ${noTokenResponse.status} without UMA challenge. Refusing public endpoint in benchmark mode.`);
            }
